...
function buildTradePlan({
  ...
  if (
    ...
    return empty(
      'NO_SETUP',
      'AVOID',
      'Struktur bearish atau breakdown. Belum ada setup long yang valid.',
    )
  }
  ...
  if (setup === 'NONE') {
    return empty(
      'NO_SETUP',
      'AVOID',
      'Belum ditemukan setup trading yang jelas.',
    )
  }
  ...
  if (setup === 'RANGE') {
    ...
    return {
      ...empty(
        'WAIT_CONFIRMATION',
        'WAIT_CONFIRMATION',
        'Harga masih berada dalam range. Tunggu breakout resistance atau pullback yang lebih jelas.',
      ),
      trigger,
    }
  }
    ...
    if (lastPrice >= trigger * 1.05) {
      return {
        ...empty(
          'WAIT_CONFIRMATION',
          'WAIT_PULLBACK',
          'Breakout sudah terjadi tetapi harga terlalu jauh dari trigger. Tunggu pullback atau retest.',
        ),
        trigger,
        invalidation,
        retestLow,
    ...
    if (
      ...
      if (
        ...
        return {
          ...empty(
            'WAIT_CONFIRMATION',
            'WAIT_PULLBACK',
            'Momentum sudah terlalu extended dari trigger. Tunggu pullback.',
          ),
          trigger,
          invalidation,
          retestLow: roundToTick(
      ...
      if (
        ...
        return empty(
          'WAIT_CONFIRMATION',
          'WAIT_CONFIRMATION',
          'Momentum ada, tetapi level invalidation belum cukup jelas.',
        )
      }
  ...
  // ------------------------------------------------------------
    ...
    if (candidateSupport == null) {
      return empty(
        'WAIT_CONFIRMATION',
        'WAIT_CONFIRMATION',
        'Belum ditemukan support lokal yang cukup dekat untuk invalidation.',
      )
    }

    const buffer = Math.max(
      tickSize(candidateSupport),
    ...
    return empty(
      'NO_SETUP',
      'AVOID',
      'Setup belum memiliki aturan entry yang valid.',
    )
  }
  ...
  if (
    ...
    return {
      ...empty(
        'WAIT_CONFIRMATION',
        'WAIT_CONFIRMATION',
        'Entry atau invalidation belum cukup jelas untuk membentuk trade plan.',
      ),
      entry,
      trigger,
      retestLow,
  ...
  if (riskPoints <= 0) {
    return empty(
      'WAIT_CONFIRMATION',
      'WAIT_CONFIRMATION',
      'Risk tidak valid karena invalidation berada di atas atau sama dengan entry.',
    )
  }
